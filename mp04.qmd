...
cat("Monthly TRS Benefit: $", round(annual_benefit/12), "\n")
...
# Set up asset allocation for Age 50 to 59 (First 10 years) and Age 60 to 74 (Next 5 years)
equity_us_alloc <- 0.47  # US Equities allocation for Age 50 to 59
equity_international_alloc <- 0.32  # International Equities allocation for Age 50 to 59
bonds_alloc <- 0.21  # Bonds allocation for Age 50 to 59

equity_us_alloc_older <- 0.34  # US Equities allocation for Age 60 to 74
equity_international_alloc_older <- 0.23  # International Equities allocation for Age 60 to 74
bonds_alloc_older <- 0.43  # Bonds allocation for Age 60 to 74

# Initialize vectors to store results
salaries <- numeric(years_of_service)
employee_contributions <- numeric(years_of_service)
employer_contributions <- numeric(years_of_service)
orp_balance <- numeric(years_of_service)

# Initial salary for the first year
salaries[1] <- initial_salary

# Loop through each year and compute monthly contributions and ORP balance
for (i in 1:years_of_service) {
  
  # Get the salary for the year (starting with initial salary for the first year)
  if (i == 1) {
    salary <- initial_salary
  } else {
    # For subsequent years, use the previous year's salary and adjust based on CPI
    september_cpi_previous_year <- combined_data$inflation_rate[which(month(combined_data$date) == 9 & year(combined_data$date) == 2008 + (i - 1))]
    september_cpi_current_year <- combined_data$inflation_rate[which(month(combined_data$date) == 9 & year(combined_data$date) == 2008 + i)]
  ...
  # Calculate monthly salary (divide the annual salary by 12)
  monthly_salary <- salary / 12
  
  # Employee contribution calculation based on salary brackets (annual contribution, divided by 12 for monthly)
  if (salary <= 45000) {
    employee_contribution <- (salary * 0.03) / 12
  } else if (salary <= 55000) {
    employee_contribution <- (salary * 0.035) / 12
  } else if (salary <= 75000) {
    employee_contribution <- (salary * 0.045) / 12
  } else if (salary <= 100000) {
    employee_contribution <- (salary * 0.0575) / 12
  } else {
    employee_contribution <- (salary * 0.06) / 12
  }

  # Employer contribution: 8% for the first 7 years, 10% after (annual contribution, divided by 12 for monthly)
  if (i <= 7) {
    employer_contribution <- (salary * 0.08) / 12
  } else {
    employer_contribution <- (salary * 0.10) / 12
  }
  ...
  # Store contributions
  employee_contributions[i] <- employee_contribution
  employer_contributions[i] <- employer_contribution
  
  # Asset allocation: Based on employee's age (years of service)
  if (i <= 10) {  # First 10 years (Age 50 to 59)
    # Calculate monthly growth rates for US Equities, International Equities, and Bonds
    us_equity_growth_rate <- 1 + (combined_data$us_equity_close[i] / combined_data$us_equity_close[i - 1] - 1) * equity_us_alloc
    intl_equity_growth_rate <- 1 + (combined_data$intl_equity_close[i] / combined_data$intl_equity_close[i - 1] - 1) * equity_international_alloc
    bond_growth_rate <- 1 + (combined_data$bond_yield[i] / 100) * bonds_alloc
    
  } else {  # Age 60 to 74
    # Calculate monthly growth rates for US Equities, International Equities, and Bonds
    us_equity_growth_rate <- 1 + (combined_data$us_equity_close[i] / combined_data$us_equity_close[i - 1] - 1) * equity_us_alloc_older
    intl_equity_growth_rate <- 1 + (combined_data$intl_equity_close[i] / combined_data$intl_equity_close[i - 1] - 1) * equity_international_alloc_older
    bond_growth_rate <- 1 + (combined_data$bond_yield[i] / 100) * bonds_alloc_older
  }
  
  # Calculate the total growth rate for the year based on asset allocation
  total_growth_rate <- us_equity_growth_rate + intl_equity_growth_rate + bond_growth_rate
  
  # Initialize the ORP balance for the first year
  if (i == 1) {
    orp_balance[i] <- total_contribution
  } else {
    # Apply compound growth and add monthly contributions
    orp_balance[i] <- orp_balance[i - 1] * total_growth_rate + total_contribution
  }
}

# Output the data with monthly contributions and ORP balance for each year
yearly_orp_data <- data.frame(
  ...
  Total_Contribution = round(employee_contributions + employer_contributions, 2),
  ORP_Balance = round(orp_balance, 2)
)

print(yearly_orp_data)
```
...
with that you would get the new salary for each year. until retirement. Now we need to use this formaula. 0.0167* FAS * years of service. so we need to get the FAS, TO GET FAS IS THE average salary for the las 3 years. 
with the last formula you would get the benefits for the program. 
