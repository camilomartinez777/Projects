...
```{r}
# Calculate values
avg_monthly_trs <- mean(results$TRS_Annual_Income) / 12
avg_monthly_orp <- mean(results$ORP_Annual_Withdrawal) / 12
monthly_income_gaps <- results$Income_Gap_Annual / 12
max_gap <- max(monthly_income_gaps)
min_gap <- min(monthly_income_gaps)

# Create a data frame for the table
summary_data <- data.frame(
  Metric = c(
    "Average Monthly Income (TRS)",
    "Average Monthly Income (ORP)",
    "Maximum Monthly Income Gap (TRS - ORP)",
    "Minimum Monthly Income Gap (TRS - ORP)"
  ),
  Value = c(
    avg_monthly_trs,
    avg_monthly_orp,
...
summary_table <- summary_data |>
  ...
  tab_header(
    title = "Comparison of TRS and ORP Income Metrics"
  ) |>
  cols_label(
...
summary_table
```
## Conclusion About the two programs
